...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx_2, dw_2, db_2 = affine_backward(dscores, cache_2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, hd in enumerate(hidden_dims):
            ...
            layer_input_dim = hd
        self.params['W%d'%(self.num_layers)] = weight_scale * np.random.rand(layer_input_dim, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        dp_cache = {} # droput cache
        
        for i in range(1,self.num_layers):
            if self.use_batchnorm:
                # Use batch normalization
                out,  ar_cache[i]= affine_bn_relu_forward(out, 
                                               self.params['W%d'%i], self.params['b%d'%i] , 
                                               self.params['gamma%d'%i], self.params['beta%d'%(i)], self.bn_params[i-1])
            else:
                out, ar_cache[i] = affine_relu_forward(out, 
                                                       self.params['W%d'%i], self.params['b%d'%i])
            
            if self.use_dropout:
                out,  dp_cache[i] = dropout_forward(out, self.dropout_param)
        # Last layer
        scores, ar_cache[self.num_layers] = affine_forward(out, self.params['W%d'%(self.num_layers)], self.params['b%d'%(self.num_layers)])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx, dw, db = affine_backward(dscores, ar_cache[self.num_layers])
        
        grads['W%d'%(self.num_layers)] = dw + self.reg * self.params['W%d'%(self.num_layers)]
        grads['b%d'%(self.num_layers)] = db
        
        for i in range(1, self.num_layers):
            loss += 0.5 * self.reg * np.sum(self.params['W%d'%(self.num_layers - i)] * self.params['W%d'%(self.num_layers - i)])
            if self.use_dropout:
                dx = dropout_backward(dx ,dp_cache[self.num_layers - i])
            if self.use_batchnorm:
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx, ar_cache[self.num_layers - i])
            else:
                dx, dw, db = affine_relu_backward(dx, ar_cache[self.num_layers - i])
            grads['W%d'%(self.num_layers - i)] = dw + self.reg * self.params['W%d'%(self.num_layers - i)]
            grads['b%d'%(self.num_layers - i)] = db
            if self.use_batchnorm:
                grads['gamma%d'%(self.num_layers - i)] = dgamma
                grads['beta%d'%(self.num_layers - i)] = dbeta
